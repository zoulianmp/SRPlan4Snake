...
void qSRPlanPathPlanModuleWidget::UpdateTraceMarkPosition()
{
	...
	if (listNode)
	{
		...
		realTracFile = std::string(vtksys::SystemTools::GetEnv("OPTIC_TRAC") + std::string("\\%d.raw", file_count));
		...
		if (opticTracFile.exists())
		{
			...
			qDebug("don't have this file%d", file_count);
		...
		if (suc)
		{
			...
			opticTracFile.close();
			rm_file.remove(realTracFile.c_str());
		}
	}
