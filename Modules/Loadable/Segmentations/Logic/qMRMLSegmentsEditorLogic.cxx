...
//----------------------------------------------------------------------------
const int qMRMLSegmentsEditorLogic::SLICE_INDEX_ROTATED = -1;
const int qMRMLSegmentsEditorLogic::SLICE_INDEX_OUT_OF_VOLUME = -2;
const int qMRMLSegmentsEditorLogic::SLICE_INDEX_NO_VOLUME = -3;
const std::string qMRMLSegmentsEditorLogic::SLICE_MODEL_NODE_NAME_SUFFIX = std::string("Volume Slice");
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::UpdateSliceNode()
{
...
void qMRMLSegmentsEditorLogic::UpdateSliceNodeFromLayout()
{
	if (this->SliceNode == 0)
	{
		return;
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::UpdateSliceCompositeNode()
{
	if (!this->GetMRMLScene())
	{
	...
	vtkMRMLSliceCompositeNode *node = qMRMLSegmentsEditorLogic::GetSliceCompositeNode(this->GetMRMLScene(), this->GetName());
...
//----------------------------------------------------------------------------
bool qMRMLSegmentsEditorLogic::EnterMRMLCallback()const
{
	return this->AddingSliceModelNodes == false;
}

//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::UpdateFromMRMLScene()
{
	this->UpdateSliceNodes();
}

//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::OnMRMLSceneNodeAdded(vtkMRMLNode* node)
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::OnMRMLSceneNodeRemoved(vtkMRMLNode* node)
{
	if (!(node->IsA("vtkMRMLSliceCompositeNode")
		|| node->IsA("vtkMRMLSliceNode")
		|| node->IsA("vtkMRMLVolumeNode")))
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::OnMRMLSceneStartClose()
{
	this->UpdateSliceNodeFromLayout();
	this->DeleteSliceModel();
}
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::OnMRMLSceneEndImport()
{
	this->SetupCrosshairNode();
}

//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::OnMRMLSceneEndRestore()
{
	this->SetupCrosshairNode();
}

//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::UpdateSliceNodes()
{
	if (this->GetMRMLScene()
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::SetupCrosshairNode()
{
	//
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::OnMRMLNodeModified(vtkMRMLNode* node)
{
	assert(node);
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::SetSliceNode(vtkMRMLSliceNode * newSliceNode)
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::SetSliceCompositeNode(vtkMRMLSliceCompositeNode *sliceCompositeNode)
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::SetBackgroundLayer(vtkMRMLSliceLayerLogic *backgroundLayer)
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::SetForegroundLayer(vtkMRMLSliceLayerLogic *foregroundLayer)
{
	// TODO: Simplify the whole set using a macro similar to vtkMRMLSetAndObserve
	if (this->ForegroundLayer)
	{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::SetLabelLayer(vtkMRMLSliceLayerLogic *labelLayer)
{
	// TODO: Simplify the whole set using a macro similar to vtkMRMLSetAndObserve
	if (this->LabelLayer)
	{
...
//----------------------------------------------------------------------------
vtkAlgorithmOutput * qMRMLSegmentsEditorLogic::GetImageDataConnection()
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::UpdateImageData()
{
	if (this->SliceNode->GetSliceResolutionMode() == vtkMRMLSliceNode::SliceResolutionMatch2DView)
	{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::UpdatePipeline()
{
	int modified = 0;
	if (this->SliceCompositeNode)
	{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::DeleteSliceModel()
{
	// Remove References
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::CreateSliceModel()
{
	if (!this->GetMRMLScene())
	{
		return;
...
//----------------------------------------------------------------------------
vtkMRMLVolumeNode *qMRMLSegmentsEditorLogic::GetLayerVolumeNode(int layer)
{
	vtkMRMLSliceNode *sliceNode = this->GetSliceNode();
	vtkMRMLSliceCompositeNode *compositeNode = this->GetSliceCompositeNode();
...
void qMRMLSegmentsEditorLogic::GetVolumeRASBox(vtkMRMLVolumeNode *volumeNode, double rasDimensions[3], double rasCenter[3])
{
...
//----------------------------------------------------------------------------
// Get the size of the volume, transformed to RAS space
void qMRMLSegmentsEditorLogic::GetVolumeSliceDimensions(vtkMRMLVolumeNode *volumeNode, double sliceDimensions[3], double sliceCenter[3])
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::GetVolumeSliceBounds(vtkMRMLVolumeNode *volumeNode, double sliceBounds[6])
{
...
double *qMRMLSegmentsEditorLogic::GetVolumeSliceSpacing(vtkMRMLVolumeNode *volumeNode)
{
	if (!volumeNode)
	{
...
void qMRMLSegmentsEditorLogic::FitSliceToVolume(vtkMRMLVolumeNode *volumeNode, int width, int height)
{
	vtkImageData *volumeImage;
	if (!volumeNode || !(volumeImage = volumeNode->GetImageData()))
	{
...
//----------------------------------------------------------------------------
// Get the size of the volume, transformed to RAS space
void qMRMLSegmentsEditorLogic::GetBackgroundRASBox(double rasDimensions[3], double rasCenter[3])
{
...
//----------------------------------------------------------------------------
// Get the size of the volume, transformed to RAS space
void qMRMLSegmentsEditorLogic::GetBackgroundSliceDimensions(double sliceDimensions[3], double sliceCenter[3])
{
	vtkMRMLVolumeNode *backgroundNode = 0;
	backgroundNode = this->GetLayerVolumeNode(0);
...
//----------------------------------------------------------------------------
// Get the spacing of the volume, transformed to slice space
double *qMRMLSegmentsEditorLogic::GetBackgroundSliceSpacing()
{
	vtkMRMLVolumeNode *backgroundNode = 0;
	backgroundNode = this->GetLayerVolumeNode(0);
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::GetBackgroundSliceBounds(double sliceBounds[6])
{
	vtkMRMLVolumeNode *backgroundNode = 0;
	backgroundNode = this->GetLayerVolumeNode(0);
...
void qMRMLSegmentsEditorLogic::FitSliceToBackground(int width, int height)
{
	vtkMRMLVolumeNode *backgroundNode = 0;
	backgroundNode = this->GetLayerVolumeNode(0);
...
void qMRMLSegmentsEditorLogic::FitSliceToAll(int width, int height)
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::FitFOVToBackground(double fov)
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::ResizeSliceNode(double newWidth, double newHeight)
{
	if (!this->SliceNode)
	{
		return;
...
//----------------------------------------------------------------------------
double *qMRMLSegmentsEditorLogic::GetLowestVolumeSliceSpacing()
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::GetLowestVolumeSliceBounds(double sliceBounds[6])
{
	vtkMRMLVolumeNode *volumeNode;
	for (int layer = 0; layer < 3; layer++)
	{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::GetSliceBounds(double sliceBounds[6])
{
	int i;
	for (i = 0; i<3; i++)
	{
...
double qMRMLSegmentsEditorLogic::GetSliceOffset()
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::SetSliceOffset(double offset)
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::StartSliceCompositeNodeInteraction(unsigned int parameters)
{
	vtkMRMLSliceCompositeNode *compositeNode = this->GetSliceCompositeNode();
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::EndSliceCompositeNodeInteraction()
{
	vtkMRMLSliceCompositeNode *compositeNode = this->GetSliceCompositeNode();
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::StartSliceNodeInteraction(unsigned int parameters)
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::SetSliceExtentsToSliceNode()
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::EndSliceNodeInteraction()
{
	if (this->SliceNode == NULL || this->SliceCompositeNode == NULL)
	{
		return;
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::StartSliceOffsetInteraction()
{
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::EndSliceOffsetInteraction()
{
	// This method is here in case we want to do something specific when
...
//----------------------------------------------------------------------------
void qMRMLSegmentsEditorLogic::SnapSliceOffsetToIJK()
{
...
//----------------------------------------------------------------------------
std::vector< vtkMRMLDisplayNode*> qMRMLSegmentsEditorLogic::GetPolyDataDisplayNodes()
{
...
//----------------------------------------------------------------------------
int qMRMLSegmentsEditorLogic::GetSliceIndexFromOffset(double sliceOffset, vtkMRMLVolumeNode *volumeNode)
{
	if (!volumeNode)
	{
...
int qMRMLSegmentsEditorLogic::GetSliceIndexFromOffset(double sliceOffset)
{
	vtkMRMLVolumeNode *volumeNode;
	for (int layer = 0; layer < 3; layer++)
	{
...
//----------------------------------------------------------------------------
vtkMRMLSliceCompositeNode* qMRMLSegmentsEditorLogic
::GetSliceCompositeNode(vtkMRMLSliceNode* sliceNode)
{
	return sliceNode ? qMRMLSegmentsEditorLogic::GetSliceCompositeNode(
...
//----------------------------------------------------------------------------
bool qMRMLSegmentsEditorLogic::IsSliceModelNode(vtkMRMLNode *mrmlNode)
{
	if (mrmlNode != NULL &&
		mrmlNode->IsA("vtkMRMLModelNode") &&
		mrmlNode->GetName() != NULL &&
		strstr(mrmlNode->GetName(), qMRMLSegmentsEditorLogic::SLICE_MODEL_NODE_NAME_SUFFIX.c_str()) != NULL)
	{
		return true;
	}
	return false;
...
//----------------------------------------------------------------------------
bool qMRMLSegmentsEditorLogic::IsSliceModelDisplayNode(vtkMRMLDisplayNode *mrmlDisplayNode)
{
