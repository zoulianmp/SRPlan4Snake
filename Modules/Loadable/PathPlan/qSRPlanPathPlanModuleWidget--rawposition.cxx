...
void qSRPlanPathPlanModuleWidget::UpdateTraceMarkPosition()
{
        ...
        if (listNode)
        {
            ...
            static unsigned int file_count = 0;
                ...
                std::string a;
                count >> a;
                //realTracFile = std::string("D:\\NetworkShare\\OpticTrace\\"+ a +std::string(".raw"));
                ...
                if (suc)
                {
                        ...
                        float x, y, z;
                        char *data_p = bytes.data();
                        file_count++;
                        while (*data_p)
                        {
                                x = *(float*)data_p;
                                y = *(float*)(data_p + sizeof(float));
                                z = *(float*)(data_p + 2 * sizeof(float));
                                data_p += 3 * sizeof(float);
